...
@Component({
  ...
  styleUrl: './admin-agence.component.scss',
})
export class AdminAgenceComponent {
  visible = false;
  AdminForm!: FormGroup;
  edit: boolean = false;
  ...
  getAdmin() {
    this.utilisateurService.getAdminA().subscribe({
      next: (data) => {
        this.adminAgence = data.map((utilisateur: any) => {
          return {
            ...utilisateur,
  ...
  bloquer(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.utilisateurService.activerAdminA(id).subscribe({
          next: () => {
  ...
  AddAdmin() {
    if (this.AdminForm.valid) {
      ...
      this.utilisateurService.ajouterAdminA(nouveauAdmin).subscribe({
        next: (res) => {
  ...
  updateAdmin(): void {
    if (this.currentUserId != null && this.AdminForm.valid) {
      ...
      this.utilisateurService
        .modifierAdminA(this.currentUserId, updatedAdmin)
        .subscribe({
          next: (res) => {
  ...
  deleteUser(id: number): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.utilisateurService.supprimerAdminA(id).subscribe({
          next: () => {
